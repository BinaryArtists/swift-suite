...
public class HudView : UIView {
    ...
    private func setupUI() {
        ...
        switch type {
        case .success:
            addImageView(image: HudViewImage.imageOfSuccess)
        case .error:
            addImageView(image: HudViewImage.imageOfError)
        case .info:
            addImageView(image: HudViewImage.imageOfInfo)
        case .loading:
            addActivityView()
        case .text:
            break
    ...
    public static func showSuccess(text:String?,delay:TimeInterval,enable:Bool = true) {
        HudView(text: text, type: .success, delay: delay,enable: enable).show()
    }

    public static func showError(text:String?,delay:TimeInterval,enable:Bool = true) {
        HudView(text: text, type: .error, delay: delay,enable:enable).show()
    }

    public static func showLoading(enable:Bool = false) {
        HudView(text: nil,type:.loading,delay: 0,enable:enable).show()
    }

    public static func showLoading(text:String?,enable:Bool = true) {
        HudView(text: text,type:.loading,delay: 0,enable:enable).show()
    }

    public static func showInfo(text:String?,delay:TimeInterval,enable:Bool = true) {
        HudView(text: text, type: .info, delay: delay,enable:enable).show()
    }

    public static func showText(text:String?,delay:TimeInterval,enable:Bool = true) {
        HudView(text: text,type:.text,delay: delay,enable:enable).show()
    }
    ...
    public func show() {
        ...
        self.animate(hide: false) {
            if self.delay > 0 {
                HudView.asyncAfter(duration: self.delay, completion: {
                    self.hide()
                })
            }
        }
    ...
    public func hide(delay:TimeInterval = delayTime) {
        HudView.asyncAfter(duration: delay) {
            self.hide()
        }
    }
...
//MARK: Class HudViewImage
private class HudViewImage {
    ...
    fileprivate class func draw(_ type: HudViewType) {
